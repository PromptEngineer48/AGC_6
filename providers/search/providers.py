    ...
    async def search(
        ...
        async with session.get(self.BASE_URL, params=params) as resp:
            if resp.status != 200:
                ...
                logger.error(f"[Google] Error {resp.status}: {text}")
...
class DirectUrlProvider(BaseSearchProvider):
    def __init__(self):
        pass
    ...
    async def search(
        ...
        # 'query' here is actually the comma-separated list of URLs from the config
        urls = [u.strip() for u in query.split(",") if u.strip()]
        return [
            SearchResult(
                title=f"Direct Link {i+1}",
                url=url,
                snippet="Content provided directly by user.",
                position=i,
            )
